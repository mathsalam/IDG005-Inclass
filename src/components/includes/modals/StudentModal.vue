            ...
            <div class="row">
              <div class="col-12">
                <div class="row">
                  <div class="col-lg-3">
                    <CropperModal
                      v-model="studentObj.photo"
                      v-model:current="currentImage"
                      v-model:error="studentErrObj.photo"
                      :width="454"
                      :height="454"
                    />
                  </div>
                  <div class="col-lg-9">
                    ...
                    <div class="row">
                      <div class="form-group col-lg-4">
                        <label>Gender</label>
                        <select
                          v-model="studentObj.gender_id"
                          class="form-control"
                          :class="{ 'is-invalid': !!studentErrObj.gender_id }"
                        >
                          <option
                            v-for="{ id, gd_kh_full } in genders"
                            :key="id"
                            :value="id"
                          >
                            {{ gd_kh_full }}
                    ...
                    <div class="row">
                      <div class="form-group col-lg-4">
                        ...
                        <select
                          v-model="studentObj.ethnicity_id"
                          class="form-control"
                          :class="{
                            'is-invalid': !!studentErrObj.ethnicity_id,
                          }"
                        >
                          <option
                            v-for="{ id, eth_kh } in ethnicities"
                            :key="id"
                            :value="id"
                          >
                            {{ eth_kh }}
                      ...
                      <div class="form-group col-lg-4">
                        ...
                        <select
                          v-model="studentObj.nationality_id"
                          class="form-control"
                          :class="{
                            'is-invalid': !!studentErrObj.nationality_id,
                          }"
                        >
                          <option
                            v-for="{ id, nat_kh } in nationalities"
                            :key="id"
                            :value="id"
                          >
                            {{ nat_kh }}
                      ...
                      <div class="form-group col-lg-4">
                        ...
                        <select
                          v-model="studentObj.religion_id"
                          class="form-control"
                          :class="{ 'is-invalid': !!studentErrObj.religion_id }"
                        >
                          <option
                            v-for="{ id, rel_kh } in religions"
                            :key="id"
                            :value="id"
                          >
                            {{ rel_kh }}
              ...
              <div class="col-12">
                ...
                <div class="row">
                  ...
                  <div class="form-group col-lg-3">
                    ...
                    <VueMultiSelect
                      v-model="selectedPobDistrict"
                      :options="pobDistricts"
                      track-by="id"
                      label="name_kh"
                      placeholder="---none---"
              ...
              <div class="col-12">
                ...
                <div class="row">
                  ...
                  <div class="form-group col-lg-3">
                    <label>District / Municipality / City</label>
                    <VueMultiSelect
                      v-model="selectedPorDistrict"
                      :options="porDistricts"
                      track-by="id"
                      label="name_kh"
                      placeholder="---none---"
...
import {
  apiGetAllGenders,
  apiGetAllNationalities,
  apiGetAllEthnicities,
  apiGetAllReligions,
} from "@/functions/api/asset";
import {
  apiGetProvinces,
  apiGetDistrictsByProvince,
  apiGetCommunesByDistrict,
  apiGetVillagesByCommune,
} from "@/functions/api/geo";
...
const selectedPobProvince = computed({
  get() {
    return (
      provinces.value.find((p) => p.id === studentObj.pob_province_id) || null
    );
  },
  set(value) {
...
const selectedPobDistrict = computed({
  get() {
    return (
      pobDistricts.value.find((d) => d.id === studentObj.pob_district_id) ||
      null
    );
  },
  set(value) {
...
const selectedPobCommune = computed({
  get() {
    return (
      pobCommunes.value.find((c) => c.id === studentObj.pob_commune_id) || null
    );
  },
  set(value) {
...
const selectedPobVillage = computed({
  get() {
    return (
      pobVillages.value.find((v) => v.id === studentObj.pob_village_id) || null
    );
  },
  set(value) {
...
const selectedPorProvince = computed({
  get() {
    return (
      provinces.value.find((p) => p.id === studentObj.por_province_id) || null
    );
  },
  set(value) {
...
const selectedPorDistrict = computed({
  get() {
    return (
      porDistricts.value.find((d) => d.id === studentObj.por_district_id) ||
      null
    );
  },
  set(value) {
...
const selectedPorCommune = computed({
  get() {
    return (
      porCommunes.value.find((c) => c.id === studentObj.por_commune_id) || null
    );
  },
  set(value) {
...
const selectedPorVillage = computed({
  get() {
    return (
      porVillages.value.find((v) => v.id === studentObj.por_village_id) || null
    );
  },
  set(value) {
  ...
  try {
    ...
    if (studentObj.id === null) {
      ...
      response = await apiUpdateStudent(
        await buildFormData(studentObj, photoChanged),
      );
    ...
    if (status === 422) {
      Object.keys(studentErrObj).forEach((key) => {
        studentErrObj[key] = data.errors[key] ? data.errors[key][0] : "";
      });
      return CloseModal();
    }
